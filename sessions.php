<div id="sescontainer">


<h1 style="font-family: 'Libre Barcode 128 Text', cursive; font-size: 92px; text-align: center; line-height: 12pt; font-weight: 400;">Sessions</h1>			
		
	
	<!-- Date 1 -->
	<div>
              <fieldset style="border: none;" class="majorpoints">
                <legend class="majorpointslegend"><b>&nbsp; Session 1 &nbsp;</b></legend>
                <div class="hidersb" style="display:none">
				
				<b>"Hide and Seek"</b>
                  <ul style="font-size: 12pt;">
				  <li class="addition">Date: May 11th, 2078</li>
				  <li class="newpage">The Group has just been promoted to Year One rank in the Xicheng Hub - with it, is access to private quarters on campus, as well as discounts to supplies and a new dossier with missions.</li>
				  <li class="bug">They accepted the first mission: Locate Franklin Barry and the missing supply cache he was supposed to bring back.</li>
				  <li class="change">The Group visited Baita Si, a local Buddhist Temple that is having troubles keeping Rolatide from buying their land to erect a new factory due to lack of funds and structural repair costs to their aging buildings. The monks pointed The Group to Lotus Blue, a new bar that some of the Hub members frequented. The Group elected to pool together their personal credits to save Baita Si before moving on to Lotus Blue.
				  <br/>
				  <br/>Franklin was at the bar, but the cache had been sold off with Baita Si's troubles in mind. After telling Franklin that they themselves have handed over the necessary funds, Franklin parted ways with Xicheng Hub, leaving behind 10k credits he made from selling the supplies <i>and</i> a cryptic message. Willard made use of his USB drive to aid his research of the origin of the message and discovered a hidden article. The Group decided to try to trace the 10k credits on Franklin's card back to the seller. The Bank of Beijing reported that the transaction traced back to The Forbidden City. 
				  <br/><br/>As they exited the Bank, an ambush unfolded..!</li>
                  </ul>
				  
				  </div>
				  </fieldset>
				  </div>
				  <!-- Date 1 - end -->
				  
				  <!-- Date 2 -->
	<div>
              <fieldset style="border: none;" class="majorpoints">
                <legend class="majorpointslegend"><b>&nbsp; Session 2 &nbsp;</b></legend>
                <div class="hidersb" style="display:none">
				
				<b>"I Get By With A Little Help From My Friends"</b>
                  <ul style="font-size: 12pt;">
				  <li class="addition">Date: May 11th, 2078</li>
				  <li class="newpage">The Group has finished the first mission!</li>
				  <li class="bug">The Group has survived the first combat!</li>
				  <li class="change">The ambush was thwarted with the help of Starbuck "Danger" Gonzales and Cornelius Quartermain III, who dropped off another Year One with the Group. They decided to leave their assailants alive, but tied up and waiting for authorities after the thugs spit out another cryptid message. Broseph "Tommy P" McBroheim accompanied the Group on their way back to the Hub to get paid for completing their first mission. Although Franklin Barry has officially cut ties with the Xicheng Hub and sold the supplies they needed, the Group did turn in the 10k Credits he handed over.
				  <br/>
				  <br/>Before calling it a night, Grandfather Clock sent the Group on a fetch quest for some scrap metal, eggs, and milk for another easy thousand credits. What a weird grocery list!
				  <br/><br/>What awaits the Group in the morning..?
				  </li>
                  </ul>
				  
				  </div>
				  </fieldset>
				  </div>
				  <!-- Date 2 - end -->
				  
				   <!-- Date 3 -->
	<div>
              <fieldset style="border: none;" class="majorpoints">
                <legend class="majorpointslegend"><b>&nbsp; Session 3 &nbsp;</b></legend>
                <div class="hidersb" style="display:none">
				
				<b>"Two Candles and Their Flame"</b>
                  <ul style="font-size: 12pt;">
				  <li class="addition">Date: May 12th, 2078</li>
				  <li class="newpage">The Group has adopted an orphan!</li>
				  <li class="bug">The Group accepted "Think of the Children"!</li>
				  <li class="change">After a good night's rest, the Group woke refreshed and ready to rumble. Diego Ku approached the group with a new mission : to investigate Jimothy Ticket's claim of "glitches" appearing at the China's Teenager and Childrens Fund, a local orphanage.
				  <br/><br/>Upon arrival, the Group found that the first floor of the orphanage was completely empty, save for a few borderline-creepy paintings of animals in victorian-era clothing, a vending machine full of soda, and the techno-natural ghostly images of two adults, a child, and a dog! After snooping around the second floor, they discover the bedroom where the orphans were all in bed - all, but one!
				  <br/><br/>Attempting to communicate with Yesenya, the child left awake, resulted in the "glitch" adults becoming waxy and disfigured and the "glitch" child growing into a huge flame! The Group was able to put out the fire of battle before too much damage was caused to the unnaturally lethargic children.
				  <br/><br/>Yesenya explained that her family had died in a fire the year prior to being taken in, though the after-images left of her parents and brother were due in part to a certain beverage being served at the orphanage. Bellamy was able to connect with the girl through his equally tragic backstory, and thanks to some <i>bamboozling</i> on Willard's part, ended up adopting Yesenya.
				  <br/><br/>The Group was able to piece together that the soda at the orphanage was actually <i>Rola-Cola</i>, Rolatide's experiment in mood altering substances. It's believed that Jimothy Ticket, the orphanage's patriarch and quest-giver, is in on this experiment in someway or another.
				  <br/><br/>The Group returns to the Hub to confront Mr. Ticket...
				  </li>
                  </ul>
				  
				  </div>
				  </fieldset>
				  </div>
				  <!-- Date 3 - end -->
				  



<script>
$(function() {
      $('.majorpoints').click(function() {
        $(this).find('.hidersb').toggle();
      });
	  
});

</script>

<style>

#sescontainer {
    background-color: rgba(222, 222, 222, 0.17);
    color: #5384ff;
    border-radius: 5px;
    padding: 5px;
    position: relative;
    font-family: Simsun;
    width: 850px;
    height: auto;
    z-index: 1;
    border: solid white 1px;
    box-shadow: rgba(0, 0, 0, .8) 0 2px 8px;
    margin: auto;
    font-size: 14pt;
    overflow-y: auto;
    top: 0px;
}

.sesparagraph {
	width: 600px;
	list-style: none;
}

.majorpointslegend {
	    position: relative;
    top: 0px;
    left: -25px;
    width: auto;
    text-align: center;
    list-style: none;
    background: #0000004f;
	font-size: 14pt;
	color: #8edc39;
	cursor: pointer;
}

</style>